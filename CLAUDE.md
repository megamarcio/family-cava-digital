...
---
## 0.1 POLÍTICA META — REFERÊNCIA OFICIAL 2026 (consultar antes de copy, URLs e eventos)
> Fontes oficiais (verificar periodicamente, mudam sem aviso):
...
> - Acompanhar updates: https://www.facebook.com/business/news + Account Quality dashboard (dentro do BM)

**1. Personal Attributes (o gatilho nº1 de reprovação deste funil)**
- Proibido copy que implique conhecimento das características pessoais do usuário: status de saúde, financeiro, político, relacionamento.
- Em 2026 a fiscalização mira IMPLICAÇÃO INDIRETA, não só afirmação explícita. É a causa mais comum de reprovação.
- Banido: 2ª pessoa tipo "você tem", "você está sofrendo de", "você luta com", "está cansado?".
- Correção: trocar linguagem de "Você" por linguagem de "Benefício/Tema". Ex: "Você está quebrado?" → "Ferramentas financeiras para orçamentos modernos". Aplicado: "qual peptídeo seu corpo pede" → "quais categorias de peptídeos existem".

**2. Health & Wellness restrita — claims proibidos**
- Banido: claim de cura de doença, antes-e-depois (saúde/peso/estética), claims de açúcar no sangue/diabetes, resultado milagroso/garantido, exagero.
- Restrito: referência a condições sensíveis de saúde + segmentação por idade.
- Saída oficial: reestruturar criativo em torno de claims FUNCIONAIS comprovados (structure-function), evitar condições restritas.

**3. Disclaimer obrigatório NO ANÚNCIO (não só na landing)**
- Todo ad de suplemento de saúde deve conter, no próprio texto do anúncio: "Este produto não se destina a diagnosticar, tratar, curar ou prevenir qualquer doença."
- Ad sem esse disclaimer = rejeição automática.
- (Adaptar pro infoproduto: "Conteúdo educativo. Não se destina a diagnosticar, tratar, curar ou prevenir qualquer doença.")

**4. Pixel / URL / nome de evento também são escaneados (CRÍTICO PRO BUILD)**
- A Meta infere atributo pessoal por SINAL, não só por label explícito. A interpretação de "implica ou infere" é ampla.
- O CAMINHO DA URL sozinho vira sinal de saúde: ex. `/blog/managing-type-2-diabetes` é classificado como sinal mesmo sem parâmetro.
- Eventos como `fertility_supplement_viewed`, `glp1_product_added` são flagueados pela camada de classificação.
- REGRA DE BUILD: URLs e eventos 100% NEUTROS. Usar `/quiz`, `/resultado`, `/guia` — NUNCA `/quiz/gordura-teimosa`. Eventos: `QuizStart`, `QuizComplete`, `Lead`, `Purchase` — NUNCA `peptideo_emagrecimento`, `lead_dor`.

**5. Auditoria retroativa (aprovado hoje ≠ seguro amanhã)**
- Campanhas ativas sofrem auditoria contínua. Ad aprovado em janeiro pode ser pausado em março se recalibrarem limites.
- Implicação: BM SEPARADO é obrigatório — recall retroativo não pode derrubar a conta principal (R3/Genesis/VPC).

**6. Comportamento da conta também conta (afeta o aquecimento)**
- IA da Meta monitora comportamento operacional, não só conteúdo: escalar budget rápido demais ou trocar de dispositivo dispara restrição.
- Subir orçamento DEVAGAR. Aquecer conta antes de escalar.

**Appeals (se reprovar)**
- Enviar educado, esperar 7-10 dias, NÃO disparar múltiplos appeals (piora o score), focar em ação corretiva específica.
---
...
- Logo/identidade: ícone PP no cabeçalho + favicon; tipografia quadrada (Chakra Petch); imagens das dores em webp leve.

**Canal de tráfego decidido:** Meta agora (Page + Instagram mínimo, sem WhatsApp) e TikTok no futuro. Follow-up por E-MAIL (não WhatsApp).

**Divergências a decidir (doc vs. realidade):**
- Gateway: doc sugere Stripe; projeto usa Globalpay/Nox (preferência do dono). Manter Globalpay/Nox salvo decisão contrária.
...
3. Disclaimer educativo no anúncio E nas páginas.
4. Pixel + Conversions API com eventos neutros.
5. Webhook de pagamento real → liberar entrega + e-mail por bucket.
