...
// Página de PROFUNDIDADE (pós-clique / "saber mais"): substância educativa que
// constrói confiança e mantém coerência com o anúncio — sem fricção na entrada.
export default function Saber() {
  ...
  return (
    <main className="mesh">
      ...
      <section className="mx-auto max-w-3xl px-5 pb-6 pt-8 text-center">
        <motion.div {...fadeUp}>
          <span className="inline-flex items-center gap-2 rounded-full glass px-3 py-1 text-xs font-medium text-lime-glow">📚 Conteúdo educativo</span>
          <h1 className="mt-5 text-3xl font-black md:text-5xl">Entenda o universo dos <span className="gradient-text">peptídeos</span></h1>
          <p className="mx-auto mt-4 max-w-xl text-white/65">
            Um panorama informativo das principais categorias estudadas pela ciência — organizado por objetivo, sem achismo e sem promessas.
          </p>
        </motion.div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16">
        <motion.div {...fadeUp} className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-white/60">Cada categoria reúne peptídeos estudados para um foco específico.</p>
        </motion.div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categorias.map(([key, meta], i) => (
            <motion.button key={key} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.05 }} onClick={goQuiz}
              ...
              <div className="p-6 pt-4">
                ...
                <span className="mt-4 inline-block text-xs font-semibold text-lime-glow opacity-0 transition group-hover:opacity-100">Explorar categoria →</span>
              </div>
            </motion.button>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-4xl px-5 py-16 text-center">
        <motion.div {...fadeUp}>
          <h2 className="text-3xl font-black md:text-5xl">Comece pelo <span className="gradient-text">quiz</span></h2>
          <p className="mx-auto mt-4 max-w-xl text-white/60">6 perguntas, menos de 1 minuto, e você sai com um panorama claro do tema.</p>
          <button onClick={goQuiz} className="btn-primary mt-8 px-10 py-5 text-lg">Fazer o quiz grátis →</button>
        </motion.div>
      </section>
